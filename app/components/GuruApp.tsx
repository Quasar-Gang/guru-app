...
export default function GuruApp() {
  ...
  const createPlan = async () => {
    ...
    try {
      ...
      const session = await api<{ session_id: string }>("/plan-sessions", { method: "POST", body: JSON.stringify({ goal: goal.trim(), ...(traitId ? { trait_role_model_id: traitId } : {}), ...(personaId ? { persona_role_model_id: personaId } : {}) }) });
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {view === "today" && <TodayView tasks={tasks} completed={completed} minutes={todayMinutes} onToggle={toggleTask} onRevision={() => setShowRevision(true)} plan={currentPlan} />}
        {view === "plan" && <PlanView plan={currentPlan} plans={plans} onCompare={() => setShowCompare(true)} onExport={exportPlan} onRevision={() => setShowRevision(true)} />}
